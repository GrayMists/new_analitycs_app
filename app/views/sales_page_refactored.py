# app/views/sales_page_refactored.py
...
def _render_charts(charts_service: SalesChartsService, df_work: pd.DataFrame, df_latest_decade: pd.DataFrame,
    ...
    tab_qty, tab_city, tab_trend = st.tabs(["Кількість по продуктах", "Виручка по містах (+ к-сть)", "Тренд по декадах "])
...
def show():
    ...
    try:
        ...
        if price_rows_count > 0 and price_assigned == 0:
            # покажемо кілька унікальних препаратів, які не знайшли ціну
